#!/usr/bin/env python3
"""Add Smart Matches CTA section at the top"""
...
.smart-matches-hero{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:16px;padding:40px;color:#fff;margin-bottom:32px;}
.cta-banner{background:linear-gradient(135deg,#f093fb 0%,#f5576c 100%);border-radius:16px;padding:32px;color:#fff;margin-bottom:32px;text-align:center;}
</style>
{% endblock %}
...
<h3 class="fw-bold mb-3"><i class="fas fa-sparkles me-2"></i>Get Personalized Apartment Matches</h3>
<p class="mb-4 fs-5">Create a free account to unlock AI-powered smart matches tailored to your preferences, budget, and lifestyle.</p>
<div class="d-flex gap-3 justify-content-center">
<a href="{% url 'register_applicant' %}" class="btn btn-light btn-lg rounded-pill px-5 fw-bold">Create Free Account</a>
<a href="{% url 'login' %}" class="btn btn-outline-light btn-lg rounded-pill px-5">Sign In</a>
</div>
</div>
{% elif smart_matches %}
<div class="smart-matches-hero mb-5">
<div class="d-flex align-items-center justify-content-between mb-4">
<div>
<h3 class="fw-bold mb-2"><i class="fas fa-sparkles me-2"></i>Your Smart Matches</h3>
<p class="mb-0 opacity-75">Personalized recommendations based on your preferences</p>
</div>
<span class="badge bg-light text-dark fs-6">Top {{ smart_matches|length }} Picks</span>
</div>
<div class="row flex-nowrap overflow-auto pb-3 gx-3" style="scrollbar-width:none;-ms-overflow-style:none;">
...
<div class="position-absolute top-0 end-0 m-2"><span class="badge bg-success shadow-sm fs-6">{{ match.score }}% Match</span></div>
<div class="position-absolute bottom-0 start-0 m-2"><span class="badge bg-dark shadow-sm">${{ match.apartment.rent_price|floatformat:0 }}</span></div>
</div>
<div class="card-body p-3">
<h6 class="fw-bold mb-1">{{ match.apartment.building.name }}</h6>
<p class="small text-muted mb-2">{{ match.apartment.building.neighborhood }}</p>
<div class="d-flex gap-2 small mb-3">
<span><i class="fas fa-bed me-1"></i>{{ match.apartment.bedrooms|floatformat:0 }}</span>
<span><i class="fas fa-bath me-1"></i>{{ match.apartment.bathrooms|floatformat:0 }}</span>
</div>
<a href="{% url 'apartment_overview' match.apartment.id %}" class="btn btn-primary btn-sm w-100 rounded-pill">View Details</a>
</div>
</div>
</div>
{% endfor %}
